...
public class CommonNavigator extends FrameLayout implements IPagerNavigator, NavigatorHelper.OnNavigatorScrollListener {
    ...
    private void initTitlesAndIndicator(int margin) {
    ...
    private boolean disChildView;
    public void disChildView(boolean disChildView){
        this.disChildView = disChildView;
    }
    private void preparePositionData1() {
        mPositionDataList.clear();
        for (int i = 0, j = mNavigatorHelper.getTotalCount(); i < j; i++) {
            PositionData data = new PositionData();
            View v = mTitleContainer.getChildAt(i);
            if (v != null) {
                View viewById = v.findViewById(R.id.msgTv);
                if (viewById != null){
                    data.mLeft = v.getLeft();
                    data.mTop = v.getTop();
                    data.mRight = v.getRight()-viewById.getWidth();
                    data.mBottom = v.getBottom();
                    if (v instanceof IMeasurablePagerTitleView) {
                        IMeasurablePagerTitleView view = (IMeasurablePagerTitleView) v;
                        data.mContentLeft = view.getContentLeft();
